...
update() {
    ...
    bash <(curl -Ls https://raw.githubusercontent.com/RyanRaw/XrayR_For_SSpanel-uim/master/install/install.sh) $version
    if [[ $? == 0 ]]; then
...
update_shell() {
    wget -O /usr/bin/XrayR -N --no-check-certificate https://raw.githubusercontent.com/RyanRaw/XrayR_For_SSpanel-uim/master/install/XrayR.sh
    if [[ $? != 0 ]]; then
        echo ""
