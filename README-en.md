# XrayR — SSPanel-UIM Edition

[![](https://img.shields.io/badge/TgChat-@XrayR讨论-blue.svg)](https://t.me/XrayR_project)
[![](https://img.shields.io/badge/Channel-@XrayR通知-blue.svg)](https://t.me/XrayR_channel)
![](https://img.shields.io/github/stars/RyanRaw/XrayR_For_SSpanel-uim)
![](https://img.shields.io/github/forks/RyanRaw/XrayR_For_SSpanel-uim)
![](https://github.com/RyanRaw/XrayR_For_SSpanel-uim/actions/workflows/release.yml/badge.svg)
![](https://github.com/RyanRaw/XrayR_For_SSpanel-uim/actions/workflows/docker.yml/badge.svg)
[![Github All Releases](https://img.shields.io/github/downloads/RyanRaw/XrayR_For_SSpanel-uim/total.svg)]()

[Iranian(farsi) README](README_Fa.md) | [Vietnamese(vi) README](README-vi.md) | [Chinese(zh) README](README.md)

A Xray backend framework that can easily support many panels.

A back-end framework based on XRAY supports V2ay, Trojan, Shadowsocks protocols, which are easy to expand and support multi-panel docking.

**This project is a fork of [XrayR-project/XrayR](https://github.com/XrayR-project/XrayR), primarily adapted for SSPanel-UIM.**

If you like this project, you can click STAR+WATCH in the upper right corner to continue to pay attention to the progress of this project.

## Guide for use

Tutorial：[Detailed tutorial](https://ryanraw.github.io/XrayR_For_SSpanel-uim/)
## Disclaimer

This project is just my personal learning and development and maintenance. I do not guarantee any availability and is not responsible for any consequences caused by the use of this software.

## Features

* Permanent open source and free.
* Support V2Ray, Trojan, Shadowsocks multiple protocols.
* Support new features such as Vless and XTLS.
* Support single instance docking multi-panel and multi-node, no need to start repeatedly.
* Support restriction online IP
* Support node port level and user level speed limit.
* The configuration is simple and clear.
* Modify the automatic restart instance.
* Easy to compile and upgrade, you can quickly update the core version and support the new features of XRAY-CORE.
* Support REALITY, XHTTP, and post-quantum features (X25519MLKEM768, ML-DSA-65).

## Transports & Crypto

### REALITY

* Supports `security: reality` (VLESS / Trojan). The server-side parameters **can be delivered by the panel**, or written locally in `REALITYConfigs` in `config.yml`.
* Value precedence: panel `reality-opts` → `reality-opts.xxx` dotted keys → flat `snake_case` → flat `camelCase` → local `REALITYConfigs` → built-in defaults. Fields the panel does not send fall back automatically.
* Tolerant of inconsistent panel formats, e.g. `"enable_reality": "true"` (string), `"reality-opts.private_key"` (dotted key), `private_key` (flat snake_case).
* `privateKey` must decode to 32 bytes; if `dest` is not provided it falls back to `sni + ":443"`; if `shortIds` is not provided the built-in default `["", "0123456789abcdef"]` is used (xray requires it to be non-empty).

### XHTTP

* Supports `network: xhttp` (same as `splithttp`); `path`, `host` and `mode` can all be delivered by the panel.
* `mode` accepts `auto` / `packet-up` / `stream-up` / `stream-one`; when omitted, xray's default `auto` is used.
* An empty server-side `host` disables Host header validation; otherwise the request Host must match exactly (case-insensitive, port ignored). For REALITY, leave it empty or set it to the SNI.

### X25519MLKEM768 (post-quantum key exchange)

* REALITY's TLS 1.3 handshake supports the X25519MLKEM768 hybrid post-quantum key exchange, protecting against "harvest now, decrypt later".
* **The server supports it automatically, no configuration needed**; whether it is negotiated depends on the client's ClientHello (uTLS fingerprint), and the server follows.
* Enable `show` on the client to confirm — the log prints:
  `REALITY localAddr: ... is using X25519MLKEM768 for TLS' communication: true`

### ML-DSA-65 (post-quantum signature)

* REALITY can use ML-DSA-65 to add an extra signature check on the server certificate, protecting REALITY authentication against future quantum computers.
* The server uses `mldsa65Seed`, the client uses the matching `mldsa65Verify`.
* `mldsa65Seed` is generated by `XrayR mldsa65` (32 bytes, `base64.RawURLEncoding`) and **must differ from `privateKey`**, otherwise xray refuses to start.
* If the client has no `mldsa65Verify` configured, the connection still works — it just skips the extra check.
* Enable `show` on the client to confirm — the log prints:
  `REALITY localAddr: ... is using ML-DSA-65 for cert's extra verification: true`
* Manager script menu `14` / `15` generates x25519 and ML-DSA-65 keypairs (press Enter for random, or paste an existing private key / seed to derive the other half).

## Function

| Function        | v2ray | trojan | shadowsocks |
|-----------|-------|--------|-------------|
| Get node information    | √     | √      | √           |
| Get user information    | √     | √      | √           |
| User traffic statistics    | √     | √      | √           |
| Server information report   | √     | √      | √           |
| Automatically apply for a TLS certificate | √     | √      | √           |
| Automatic renewal TLS certificate | √     | √      | √           |
| Number of online people    | √     | √      | √           |
| Online user restrictions    | √     | √      | √           |
| Audit rules      | √     | √      | √           |
| Node port speed limit    | √     | √      | √           |
| According to user speed limit    | √     | √      | √           |
| Custom DNS    | √     | √      | √           |

## Support for panels

| Panel                                                     | v2ray | trojan | shadowsocks             |
|--------------------------------------------------------|-------|--------|-------------------------|
| sspanel-uim                                            | √     | √      | √ (Single-ended multi-user and V2Ray-Plugin) |

## Software Installation

### 1-Click installation

```bash
bash <(curl -Ls https://cdn.jsdelivr.net/gh/RyanRaw/XrayR_For_SSpanel-uim@master/install/install.sh)
```

### Docker

[Docker deployment tutorial](https://ryanraw.github.io/XrayR_For_SSpanel-uim/xrayr-xia-zai-he-an-zhuang/install/docker)

### Manual installation

[Manual installation tutorial](https://ryanraw.github.io/XrayR_For_SSpanel-uim/xrayr-xia-zai-he-an-zhuang/install/manual)

## Configuration file and detailed use tutorial

[Detailed tutorial](https://ryanraw.github.io/XrayR_For_SSpanel-uim/)

## Thanks

* [Project X](https://github.com/XTLS/)
* [V2Fly](https://github.com/v2fly)
* [VNet-V2ray](https://github.com/ProxyPanel/VNet-V2ray)
* [Air-Universe](https://github.com/crossfw/Air-Universe)
* [XrayR-project/XrayR](https://github.com/XrayR-project/XrayR) — upstream project

## Licence

[Mozilla Public License Version 2.0](https://github.com/RyanRaw/XrayR_For_SSpanel-uim/blob/master/LICENSE)

## Telgram

[Xrayr back-end discussion](https://t.me/XrayR_project)

[Xrayr notification](https://t.me/XrayR_channel)

## Stargazers over time

[![Stargazers over time](https://starchart.cc/XrayR-project/XrayR.svg)](https://starchart.cc/XrayR-project/XrayR)
